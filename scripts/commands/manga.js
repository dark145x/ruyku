...
module.exports.run = async ({ api, event, args }) => {
  try {
    ...
    mangas += "╰──────────────⧕\nhandleReply with the number of the manga info you want to see.";
...
module.exports.handleReply = async ({ api, event, handleReply, Users }) => {
  const { type, author, mangas, chapterIDs, totalPages, chapterIndex = 0 } = handleReply;
  if (event.senderID !== author) return;
  const authorName = (await Users.getData(author)).name;
  if (type === "select_manga") {
    const choice = parseInt(event.body);
    if (isNaN(choice) || choice < 1 || choice > mangas.length) {
      return api.sendMessage("Invalid choice, please handleReply with a valid number.", event.threadID, event.messageID);
    }
    try {
    ...
    const choice = parseInt(event.body);
const selectedChapter = handleReply.chapters.find(item => item.title === choice.toString());
    if (choice < 1 || !selectedChapter) {
      return api.sendMessage("Invalid chapter number, please handleReply with a valid number.", event.threadID, event.messageID);
    }
 try {
