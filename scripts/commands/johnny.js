module.exports.config = { 
  premium: false,  prefix: true,
	...
	permission: 0,
	credits: "SAKIBIN",
	description: "put a text/msg after the command",
	category: "Meme",
	usages: "johnny [text]",
	cooldowns: 10,
	dependencies: {
...
module.exports.run = async function({ api, event, args }) {
 if (this.config.credits != 'SAKIBIN') {
	...
	let getPorn = (await axios.get(`https://i.imgur.com/9qBHSp7.png`, { responseType: 'arraybuffer' })).data;
	fs.writeFileSync(pathImg, Buffer.from(getPorn, 'utf-8'));
	let baseImage = await loadImage(pathImg);
	let canvas = createCanvas(baseImage.width, baseImage.height);
	let ctx = canvas.getContext("2d");
	ctx.drawImage(baseImage, 0, 0, canvas.width, canvas.height);
	ctx.font = "500 50px Roboto";
	ctx.fillStyle = "#FFFFFF";
	ctx.textAlign = "start";
	let fontSize = 1000;
	while (ctx.measureText(text).width > 2600) {
		fontSize--;
		ctx.font = `500 ${fontSize}px Roboto, Roboto`;
	}
	const lines = await this.saikiiWrap(ctx, text, 500);
